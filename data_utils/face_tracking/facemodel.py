...
# Ensure compatibility with pickled numpy objects referencing numpy._core.
...
class Face_3DMM(nn.Module):
    def __init__(self, modelpath, id_dim, exp_dim, tex_dim, point_num, device=None):
        ...
        for i in range(3):
            mu[:, i] -= np.mean(mu[:, i])
        mu = mu.reshape(-1)
        self.base_id = torch.as_tensor(base_id).to(self.device) / 100000.0
        self.base_exp = torch.as_tensor(base_exp).to(self.device) / 100000.0
        self.mu = torch.as_tensor(mu).to(self.device) / 100000.0
        base_tex = DMM_info["b_tex"][:tex_dim, :]
        mu_tex = DMM_info["mu_tex"]
        self.base_tex = torch.as_tensor(base_tex).to(self.device)
        self.mu_tex = torch.as_tensor(mu_tex).to(self.device)
        sig_id = DMM_info["sig_shape"][:id_dim]
        sig_tex = DMM_info["sig_tex"][:tex_dim]
        sig_exp = DMM_info["sig_exp"][:exp_dim]
        self.sig_id = torch.as_tensor(sig_id).to(self.device)
        self.sig_tex = torch.as_tensor(sig_tex).to(self.device)
        self.sig_exp = torch.as_tensor(sig_exp).to(self.device)

        keys_info = np.load(
            os.path.join(modelpath, "keys_info.npy"), allow_pickle=True
        ).item()
        self.keyinds = torch.as_tensor(keys_info["keyinds"]).to(self.device)
        self.left_contours = torch.as_tensor(keys_info["left_contour"]).to(self.device)
        self.right_contours = torch.as_tensor(keys_info["right_contour"]).to(self.device)
        self.rigid_ids = torch.as_tensor(keys_info["rigid_ids"]).to(self.device)
